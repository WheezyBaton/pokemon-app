    ...
    <>
      <section className="form">
        <div>
          ...
          <button id="search_btn" onClick={() => handleAddSearchParam("name", document.getElementById("search_input").value)}>
            Search
          </button>
        </div>
        <div>
        ...
        <div className="limit">
          <label>
            ...
            <input defaultValue={20} placeholder="Limit" id="limit" type="number" min={1} max={20} />
          </label>
        </div>
      </section>
        ...
        {pokemons.map((pokemon) => {
          ...
          return (
            <li className="pokemon" key={pokemon.id}>
              <p>{pokemon.name}</p>
              <Link href={`/pokemon/${pokemon.id}`}><img src={pokemon.pokemonUrl} alt="Pokemon Image" width={100} height={100}/></Link>

              {pokemonsId.includes(pokemon.id) ? (
                <FaStar onClick={() => handleRemoveFromLocalStorage(pokemon.id)} className="fav_star_rem" />
              ) : (
                <FaRegStar onClick={() => handleAddToLocalStorage(pokemon.id)} className="fav_star_add" />
              )}

              {!isComparing && !isCompareLimitReached && (
                  <IoIosAddCircle onClick={() => handleAddToLocalCompareStorage(pokemon.id)} className="io_ios_circle" />
              )}
              {isComparing && (
                  <IoIosCloseCircle onClick={() => handleRemoveFromLocalCompareStorage(pokemon.id)} className="io_ios_circle" />
              )}
            </li>
          );
        })}
